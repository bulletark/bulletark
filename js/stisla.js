$(function() {
	...
	$("[data-bg-t]").each(function() {
		let $this = $(this),
				$bg = $this.attr("data-bg-t");
	...
	$("[data-bg-x]").each(function() {
		let $this = $(this),
				$bg = $this.attr("data-bg-x");
		...
		$this.prepend("<div class='overlay-two'></div>");
	});
	$("[data-bg-n]").each(function() {
		let $this = $(this),
				$bg = $this.attr("data-bg-n");
	...
	$(window).scroll(function() {
		...
		if($this.scrollTop() > $(".hero").outerHeight() - 150) {
			...
			$(".navbar-brand").addClass("logo_menu");
		}else{
			$(".main-navbar").removeClass("bg-dark");
			$(".navbar-brand").removeClass("logo_menu");
		}
